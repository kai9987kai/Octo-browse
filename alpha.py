...
Required packages-install using sudo pip3 install package-name:
    - PyQt6
    - PyQt6-WebEngine
    - opencv-python
    - gTTS
...
API keys used:
- OpenWeatherMap (weather): Free tier available at https://openweathermap.org/
- NewsAPI (news): Free tier available at https://newsapi.org/
- OpenAI (summarization): Requires signup at https://openai.com/
Replace the API keys below with your own after signing up.
"""
...
import validators  # New: for URL validation
...
WEATHER_API_KEY = "2e4c66e6f2ae6b3e5e5e5e5f5e5e5e5e"  # Example OpenWeatherMap key
NEWS_API_KEY = "e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5"     # Example NewsAPI key
OPENAI_API_KEY = "sk-your-openai-key-here"               # Replace with your OpenAI key

# --------------------- Ad Blocker Interceptor ---------------------

AD_BLOCK_LIST = {
    "doubleclick.net", "googleadservices.com", "googlesyndication.com", "adservice.google.com",
    "ads.youtube.com", "ad.doubleclick.net", "adform.net", "adnxs.com", "adsrvr.org", "advertising.com",
    "amazon-adsystem.com", "scorecardresearch.com", "2mdn.net", "adzerk.net", "taboola.com", "outbrain.com",
    "pubmatic.com", "rubiconproject.com", "openx.net", "criteo.com", "ads.pubmatic.com", "ads.linkedin.com",
    "ads.facebook.com", "ads.twitter.com",
}
...
class PasswordManager:
    ...
    def __init__(self):
        ...
        self.passwords = {}  # New: Store passwords by URL
    ...
    def save_password(self, url: str, password: str):
        self.passwords[url] = self.encrypt(password)

    def get_password(self, url: str) -> str:
        if url in self.passwords:
            return self.decrypt(self.passwords[url])
        return None
...
class SettingsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.weather_location_edit = QLineEdit()  # New: Custom weather location
    ...
    def save_settings(self):
        ...
        if homepage_url and validators.url(homepage_url):  # Validate URL
            self.parent().custom_homepage = homepage_url
        if openai_key:
            self.parent().openai_api_key = openai_key
        if weather_location:
            self.parent().weather_location = weather_location
        self.accept()
...
class OctoBrowse(QMainWindow):
    def __init__(self):
        ...
        self.weather_location = "London"  # New: Default weather location
        self.default_user_agent = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) OctoBrowse/1.0"
        ...
        self.notes_sidebar.setPlaceholderText("Take notes or chat with AI here...")
        ...
        self.weather_widget.show()  # Now visible by default
    ...
    def load_settings(self):
        ...
        try:
            with open("octobrowse_settings.json", "r") as f:
                settings = json.load(f)
                self.custom_homepage = settings.get("homepage", self.custom_homepage)
                self.openai_api_key = settings.get("openai_key", self.openai_api_key)
                self.weather_location = settings.get("weather_location", self.weather_location)
    ...
    def save_settings(self):
        ...
        settings = {
            "homepage": self.custom_homepage,
            "openai_key": self.openai_api_key,
            "weather_location": self.weather_location,
            "bookmarks": self.bookmarks,
            "history": self.history if not self.incognito_mode else []
        }
        with open("octobrowse_settings.json", "w") as f:
    ...
    def toggle_incognito_mode(self):
        ...
        if self.incognito_mode:
            ...
            QMessageBox.information(self, "Incognito Mode", "Incognito mode enabled. History won't be saved.")
        else:
    ...
    def create_toolbar(self):
        ...
        back_btn.setToolTip("Back (Backspace)")
        ...
        self.url_bar.setPlaceholderText("Enter URL or search term... (Ctrl+L to focus)")
        ...
        zoom_in_btn.setToolTip("Zoom In (Ctrl+Plus)")
        zoom_in_btn.triggered.connect(self.zoom_in)
        self.toolbar.addAction(zoom_in_btn)

        zoom_out_btn = QAction("🔎", self)
        zoom_out_btn.setToolTip("Zoom Out (Ctrl+Minus)")
        ...
        chatbot_btn.setToolTip("Open AI Chatbot")
        ...
        # New: Password manager button
        password_btn = QAction("🔒", self)
        password_btn.setToolTip("Manage Passwords")
        password_btn.triggered.connect(self.manage_passwords)
        self.toolbar.addAction(password_btn)
    ...
    def add_tab(self, url: QUrl, title: str):
        ...
        browser.titleChanged.connect(lambda title, browser=browser: self.tabs.setTabText(self.tabs.indexOf(browser), title[:20]))
    ...
    def navigate_to_url(self):
        ...
        if not url_text.startswith(("http://", "https://")):
            if validators.url("https://" + url_text):
    ...
    def toggle_dark_mode(self):
        ...
        self.set_theme(self.current_theme)  # Reapply theme to ensure consistency
    ...
    def set_theme(self, theme: str):
        ...
        if theme == "dark":
            ...
            self.dark_mode = False
            self.setStyleSheet("background-color: #e6f3ff; color: #333;")
        elif theme == "custom":
    ...
    def create_custom_theme(self):
        ...
        if color.isValid():
            ...
            self.setStyleSheet(f"background-color: {self.custom_theme}; color: #fff if QColor('{self.custom_theme}').lightness() < 128 else #333;")
            self.dark_mode = QColor(color).lightness() < 128
    ...
    def upscale_page(self):
        try:
            ...
            if current_browser:
                ...
                upscaled_img = cv2.resize(img, None, fx=2, fy=2, interpolation=cv2.INTER_LANCZOS4)  # Improved interpolation
                with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as temp_file:
    ...
    def zoom_in(self):
        current_browser = self.tabs.currentWidget()
        if current_browser:
            current_browser.setZoomFactor(min(current_browser.zoomFactor() + 0.1, 3.0))  # Max zoom limit

    def zoom_out(self):
        current_browser = self.tabs.currentWidget()
        if current_browser:
            current_browser.setZoomFactor(max(current_browser.zoomFactor() - 0.1, 0.1))  # Min zoom limit

    def read_aloud(self):
        try:
            current_browser = self.tabs.currentWidget()
            if current_browser:
                current_browser.page().toPlainText(lambda text: self.speak_text(text[:1000]))  # Limit text length
        except Exception as e:
    ...
    def speak_text(self, text: str):
        try:
            ...
            QTimer.singleShot(5000, lambda: os.remove(output_file))
        except Exception as e:
    ...
    def summarize_page(self):
        try:
            ...
            current_browser = self.tabs.currentWidget()
            if current_browser:
                current_browser.page().toPlainText(lambda text: self.generate_summary(text[:4000]))  # Limit text length
        except Exception as e:
    ...
    def open_chatbot(self):
        if not self.notes_sidebar.isVisible():
            self.notes_sidebar.show()
        self.notes_sidebar.setText("Ask me anything about the current page...")
        current_browser = self.tabs.currentWidget()
        if current_browser:
            current_browser.page().toPlainText(lambda text: self.notes_sidebar.append(f"\nType your question below (page context available):"))

    def process_chatbot_query(self, query: str):
        try:
            current_browser = self.tabs.currentWidget()
            if current_browser:
                current_browser.page().toPlainText(lambda text: self.generate_chatbot_response(query, text[:4000]))
        except Exception as e:
            self.notes_sidebar.append(f"\nError: {e}")

    def generate_chatbot_response(self, query: str, page_text: str):
        try:
            openai.api_key = self.openai_api_key
            response = openai.Completion.create(
                model="text-davinci-003",
                prompt=f"Based on the following page content:\n\n{page_text}\n\nAnswer this question: {query}",
                max_tokens=150,
                temperature=0.7
            )
    ...
    def voice_command(self):
        try:
            ...
            QMessageBox.critical(self, "Error", "Speech recognition service unavailable.")
        except Exception as e:
    ...
    def change_user_agent(self):
        user_agent, ok = QInputDialog.getText(self, "Change User Agent", "Enter the new user agent:", text=self.profile.httpUserAgent())
    ...
    def update_weather(self):
        try:
            response = requests.get(f"https://api.openweathermap.org/data/2.5/weather?q={self.weather_location}&appid={WEATHER_API_KEY}&units=metric")
    ...
    def update_news(self):
        try:
            ...
            for article in data.get("articles", [])[:5]:  # Limit to 5 articles
                ...
                self.news_sidebar.addItem(QListWidgetItem(title))
        except Exception as e:
            self.news_sidebar.clear()
    ...
    def open_settings(self):
        ...
        dialog.openai_key_edit.setText(self.openai_api_key)
        dialog.weather_location_edit.setText(self.weather_location)
        dialog.exec()
        self.save_settings()
        self.update_weather()
    ...
    def show_context_menu(self, position):
        ...
        new_tab_action.triggered.connect(self.open_in_new_tab)
    ...
    def run_extension(self):
        try:
            ...
            exec(code, {"browser": self, "current_tab": self.tabs.currentWidget()})
            QMessageBox.information(self, "Extension", "Extension executed successfully.")
        except Exception as e:
    ...
    def toggle_bookmarks(self):
        self.bookmarks_sidebar.setVisible(not self.bookmarks_sidebar.isVisible())
    ...
    def manage_passwords(self):
        ...
        if current_browser:
            ...
            layout = QFormLayout()
            password_input = QLineEdit(password or "")
            layout.addRow("Password:", password_input)
            save_btn = QPushButton("Save")
            save_btn.clicked.connect(lambda: self.password_manager.save_password(url, password_input.text()))
            layout.addRow(save_btn)
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key.Key_F11:
            ...
            self.zoom_out()
        elif event.key() == Qt.Key.Key_Return and self.notes_sidebar.hasFocus():
            query = self.notes_sidebar.toPlainText().split("\n")[-1]
            if query:
