...
def select_context_chunks(
    ...
    *,
    ...
    if mode == "summary":
        # A few extra candidates let selection skip unusually expensive
        # metadata while keeping page-size-independent sampling work for the
        # normal, small max_chunks values used by the browser.
        candidate_limit = min(len(unique_chunks), max_chunks * 4)
        candidate_order = _broad_coverage_order(
            unique_chunks, max_candidates=candidate_limit
        )
    else:
...
def _broad_coverage_order(
    chunks: Sequence[SourceChunk], *, max_candidates: int
) -> list[SourceChunk]:
    """Return a bounded beginning/end/midpoint-first sample.

    The old implementation repeatedly rescanned every chunk and every selected
    index, making long-page summaries cubic in the number of chunks. In one
    dimension, the same deterministic farthest-point order can be generated by
    splitting the largest remaining interval. Work is therefore bounded by the
    requested candidate count rather than the full page size.
    """

    count = len(chunks)
    limit = min(count, max(0, int(max_candidates)))
    if not limit:
        return []
    ...
    def add_interval(left: int, right: int) -> None:
        ...
        # Largest distance wins; ties retain the old lower-index preference.
        heapq.heappush(intervals, (-distance, candidate, left, right))

    add_interval(0, count - 1)
    while len(order) < limit and intervals:
        _negative_distance, candidate, left, right = heapq.heappop(intervals)
        order.append(chunks[candidate])
